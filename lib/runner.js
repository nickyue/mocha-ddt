...
// TODO: this way for exchanging info with reporter needs to be formalized
const temp = require("./testspace");
...
function _mochaRunPromisify(mocha) {
...
_Runner.prototype.run = deasync(async function (cb) {
    ...
    if (this._tasks.length >= 1) {
        ...
        while (currentTaskIndex < runner._tasks.length) {
            ...
            for (let j = 0; j < mocha.files.length; j++) {
                let p = path.resolve(path.join(__dirname, "../", mocha.files[j]));
            ...
            await _mochaRunPromisify(mocha);
            currentTaskIndex += 1;
        }
