...
_Runner.prototype.run = function () {
    ...
    if (this._tasks.length >= 1) {
        ...
        while (currentTaskIndex < runner._tasks.length) {
            ...
            var report = currentTask.run(mocha, runner, runner._testDir);
...
runner.input = function (scope, attributeName, defaultValue) {
    ...
    if (!currentTask) {
        var fullTitle = (scope.fullTitle) ? scope.fullTitle() : scope.test.fullTitle();
        runner._addMessage("warn", `${fullTitle}: Runner context is not defined. Possibly because test is run from mocha interface. Using default values for inputs.`);
